...
smtp_user = "xx@qq.com"  # 发件人邮箱
smtp_password = "xxxxx"  # 发件人邮箱密码
recipient = "xx@qq.com"  # 收件人邮箱
...
free_gpu_index = 5  # 指定所需GPU索引（从0开始）
...
logging_handler = logging.handlers.TimedRotatingFileHandler(
    filename=log_file, when="D", backupCount=1
)
...
def check_gpu():
    ...
    gpu_utilizations = [
        int(x.strip().split(" ")[0]) for x in res.stdout.strip().split("\n")[1:]
    ]
